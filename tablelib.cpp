...
static int LoadJson(lua_State * L) {
	...
	json_value * json_val = parser.parse();
	if (json_val) {
		parser.json_to_lua(L, json_val);
		parser.free_json(json_val);
		return 1;
	} else {
		lua_pushnil(L);
		lua_pushstring(L, "invalid json format");
		return 2;
	}
	return 0;
}
...
static void tojson(lua_State * L, int index, std::string& str, std::set<const void*>& visited) {
	...
	try {
		...
		while (lua_next(L, index)) {
			...
			if (ktype == LUA_TNUMBER) {
				...
				if (lua_isinteger(L, -2)) {
					ADD_VALUE(L, integer, -2, "\"%lld\"", str);
					ADD_VALUE(L, integer, -2, "\"%lld\",", intKeys); 
				} else if (lua_isnumber(L, -2)) {
					ADD_VALUE(L, number, -2, "\"%0.6lf\"", str);
					ADD_VALUE(L, number, -2, "\"%0.6lf\",", intKeys);
				}
			} else if (ktype == LUA_TSTRING) {
				str.push_back('"');
				str += lua_tostring(L, -2);
			...
			if (vtype == LUA_TTABLE) {
				...
				if (visited.count(p)) {
					...
					visited.insert(lua_topointer(L, -1));
					tojson(L, lua_gettop(L), str, visited);
				}
			} else if (vtype == LUA_TBOOLEAN) {
